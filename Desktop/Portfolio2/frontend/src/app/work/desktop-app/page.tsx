...
export default function DesktopAppPage() {
  const [desktopProjects, setDesktopProjects] = useState<ProjectData[]>([]);
  ...
  useEffect(() => {
    async function loadProjects() {
      try {
        ...
        // Get projects with category "Desktop App"
        const desktopApps = allProjects.filter(project => project.category === "Desktop App");
        setDesktopProjects(desktopApps);
      } catch (error) {
    ...
    <div className="min-h-full">
      <div className="p-6 md:p-10 pb-20">
        ...
        <nav className="text-[11px] uppercase tracking-[0.12em] text-black/60 mb-8">
          ...
          <span className="mx-2">&gt;</span>
          <span className="text-black font-medium">DESKTOP APP</span>
        </nav>

        {/* Header */}
        <div className="mb-12">
          <h1 className="text-4xl md:text-5xl font-light tracking-tight mb-6">Desktop Applications</h1>
          <p className="text-lg text-black/70 max-w-2xl">
            Native desktop applications built with various frameworks and programming languages.
          </p>
        </div>
